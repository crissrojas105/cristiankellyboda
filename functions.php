...
function jackrose_tgmpa_register() {
	$plugins = array(
		...
		array(
			'name'               => esc_html__( 'Form Vibes – Database Manager for Forms', 'jackrose' ),
			'slug'               => 'form-vibes',
			'required'           => false,
...
function jackrose_enqueue_scripts() {
	...
	// Libraries.
	wp_register_script( 'inview', JACKROSE_JS . '/jquery.inview.min.js', array( 'jquery' ), '1.0.1', true );
...
function jackrose_editor_style() {
	...
	$args[ 'color_button_bg' ] = urlencode( jackrose_get_theme_mod( 'color_button_bg' ) );
	$args[ 'color_button_text' ] = urlencode( jackrose_get_theme_mod( 'color_button_text' ) );

	$args[ 'color_button_bg_hover' ] = urlencode( jackrose_get_theme_mod( 'color_button_bg_hover' ) );
	$args[ 'color_button_text_hover' ] = urlencode( jackrose_get_theme_mod( 'color_button_text_hover' ) );
...
function jackrose_hero_slider( $attachments ) {
	$args = array(
		...
		'fields'        => array(
			...
			array(
				'name'    => 'disable_video_mobile',
				'type'    => 'select',
				'label'   => esc_html__( 'Disable video on mobile devices.', 'jackrose' ),
				'meta'    => array(
					'options' => array(
						1 => esc_html__( 'yes', 'jackrose' ),
						0 => esc_html__( 'no', 'jackrose' ),
...
function jackrose_custom_wpkses_post_tags( $tags, $context ) {
	if ( 'post' === $context ) {
		$tags['iframe'] = array(
			'src'             => true,
			'height'          => true,
...
add_filter( 'wp_kses_allowed_html', 'jackrose_custom_wpkses_post_tags', 10, 2 );